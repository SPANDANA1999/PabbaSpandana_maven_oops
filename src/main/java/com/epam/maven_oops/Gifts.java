...
public static int Count(chocolate[] choco,int min,int max) {
int count=0;
...
choco=new chocolate[total_choco];
   
gift[i]=new Gifts(childname,total_sweets,total_choco);
...
type=sc.next();
num=sc.nextInt();
choco[j]=new chocolate(num,cost,weight,type);
tw+=weight*num;
}
